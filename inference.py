...
delta.load_state_dict(torch.load("data/model-15.pt"))
delta.eval()
...
for i in range(len(prompt_tokenized), 384):
    # (1, L, P) -> (P)
    next_token_dist = delta.forward_tokens(prompt_vector)[0, i - 1, :].exp().cumsum(dim=0)

    selected_token_index = torch.searchsorted(next_token_dist, torch.rand(1, device="cuda")).clamp(min=0, max=tokenizer.max_token_value)
